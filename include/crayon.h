...
struct Crayon
{
    ...
    Crayon(const std::string& run_uuid,
      ...
      , run_uuid(run_uuid)
    {
        ...
        auto r =
          cpr::Post(cpr::Url(url.str()),
                    cpr::Header{ { "Content-Type", "application/json" } },
                    cpr::Body{ payload.dump() });
        // std::cerr << r.text << "\n";
    }

    cpr::Url get_url(const std::string& name) const
    {
        std::stringstream url;
